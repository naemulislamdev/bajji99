                ...
                icons: {
                  ...
                  <span class="fa fa-calendar"></span>
                        ...
                        onImageUpload: function(files, editor, welEditable) {
                            ...
                            @endif
                        @endif
                         @if($WebmasterSection->url_status)
                            @foreach(Helper::languagesList() as $ActiveLanguage)
                                @if($ActiveLanguage->box_status)
                                    <div class="form-group row">
                                        <label
                                            class="col-sm-2 form-control-label">Button URL {!! @Helper::languageName($ActiveLanguage) !!}
                                        </label>
                                        <div class="col-sm-10">
                                            {!! Form::text('url_link',$Topics->url_link, array('placeholder' => '','class' => 'form-control','required'=>'', 'dir'=>@$ActiveLanguage->direction)) !!}
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                        @endif
                          @if($WebmasterSection->whatsap_status)
                            @foreach(Helper::languagesList() as $ActiveLanguage)
                                @if($ActiveLanguage->box_status)
                                    <div class="form-group row">
                                        <label
                                            class="col-sm-2 form-control-label">WhatsAPP No
                                        </label>
                                        <div class="col-sm-10">
                                            {!! Form::text('whatsap_no',$Topics->whatsap_no, array('placeholder' => 'WhatsAPP NO','class' => 'form-control','required'=>'', 'dir'=>@$ActiveLanguage->direction)) !!}
                                        </div>
                                    </div>
                                @endif
                            @endforeach
